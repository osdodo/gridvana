...
impl<'a> TerminalView<'a> {
            ...
            iced::keyboard::Event::KeyPressed {
                ...
                Key::Character(k) => {
                    ...
                    if binding_action == BindingAction::Ignore {
                        if let Some(c) = text {
                            return Self::printable_input(state, c);
                        }
                    }
                },
                Key::Named(code) => {
    ...
    fn printable_input(
        ...
        (!state.has_active_preedit())
            .then(|| Command::Write(text.as_bytes().to_vec()))
    }
    ...
    fn handle_input_method_event(
        ...
        match event {
            ...
            input_method::Event::Commit(text) if state.is_focused() => {
                state.preedit = None;
                Some(Command::Write(text.as_bytes().to_vec()))
            },
            input_method::Event::Commit(_) => None,
    ...
    fn input_method<'b>(
        ...
        InputMethod::Enabled {
            cursor: Rectangle::new(
                Point::new(
                    layout.position().x + cursor.column.0 as f32 * cell_width,
                    layout.position().y
    ...
    fn update(
        ...
        let commands = match event {
            ...
            iced::Event::InputMethod(input_method_event) => {
                if state.is_focused()
                    || matches!(input_method_event, input_method::Event::Closed)
                {
                    let command = Self::handle_input_method_event(
                        state,
                        input_method_event,
                        iced_core::Pixels(self.term.font.size),
                    );
                    shell.request_redraw();
        ...
        if matches!(
            event,
            iced::Event::Window(iced_core::window::Event::RedrawRequested(_))
        ) {
...
impl TerminalViewState {
    ...
    fn has_active_preedit(&self) -> bool {
        self.preedit
            .as_ref()
...
mod tests {
    ...
    mod input_method_tests {
        ...
        #[test]
        fn commit_writes_utf8_text() {
            let mut state = TerminalViewState::new();
            state.focus = true;

            let command = TerminalView::handle_input_method_event(
                &mut state,
                &input_method::Event::Commit("中文".to_owned()),
                iced_core::Pixels(14.0),
            );

            assert!(matches!(
                command,
                Some(Command::Write(bytes)) if bytes == "中文".as_bytes()
            ));
        }

        #[test]
        fn preedit_stores_content_and_selection() {
            let mut state = TerminalViewState::new();
            state.focus = true;

            TerminalView::handle_input_method_event(
                &mut state,
                &input_method::Event::Preedit("zhong".to_owned(), Some(1..4)),
                iced_core::Pixels(16.0),
            );

            let preedit = state.preedit.as_ref().unwrap();
            assert_eq!(preedit.content, "zhong");
            assert_eq!(preedit.selection, Some(1..4));
            assert_eq!(preedit.text_size, Some(iced_core::Pixels(16.0)));
        }
        ...
        #[test]
        fn printable_text_is_written_without_active_preedit() {
            let state = TerminalViewState::new();

            let command = TerminalView::printable_input(&state, "a");

            assert!(matches!(
                command,
                Some(Command::Write(bytes)) if bytes == b"a"
            ));
        }

        #[test]
        fn printable_text_is_suppressed_during_composition() {
            let mut state = TerminalViewState::new();
            state.preedit = Some(input_method::Preedit {
                content: "zhong".to_owned(),
                selection: None,
            ...
            assert!(TerminalView::printable_input(&state, "g").is_none());
        }
    }
